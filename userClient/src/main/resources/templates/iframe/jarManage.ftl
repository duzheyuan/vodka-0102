    ...
    <div class="row clearfix">
        <div class="col-md-12 column">
            <table
                    id="id"
                    data-toggle="table"
                    data-url="/File/find/manageJarNews"
                    data-striped="true"
...
<script>
    ...
    function optFormatter(value, row, index) {
            var del = '<button type="button" class="btn btn-danger del" name="guahao" onclick=""><i class="glyphicon glyphicon-plus"></i> 删除(谨慎使用)</button>';
            return del;
    }
    var optEvents = {
        "click .del": function (event, value, row) {
            $.ajax({
                url: '/File/delete?jnID=' + row.jnID,
